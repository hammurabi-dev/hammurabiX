...
template <int dim, typename T> class hvec {
protected:
  std::vector<T> ele;
...
public:
  ...
  // 2D
  hvec<dim, T>(const T &x, const T &y) {
    assert(dim == 2);
    this->ele.push_back(x);
    this->ele.push_back(y);
  }
  // 3D
  hvec<dim, T>(const T &x, const T &y, const T &z) {
    assert(dim == 3);
    this->ele.push_back(x);
    this->ele.push_back(y);
  ...
  // push back
  void push_back(const T &s) { this->ele.push_back(s); }
  // copy ctor
  hvec<dim, T>(const hvec<dim, T> &v) { this->ele = v.content(); }
  // move ctor
  hvec<dim, T>(hvec<dim, T> &&v) : ele(std::move(v.content())) {}
  // copy assign
  ...
  // operator +
  template <typename R> hvec<dim, T> operator+(const hvec<dim, R> &v) const {
    hvec<dim, T> tmp(*this);
  ...
  // play with caution
  template <typename R> hvec<dim, T> &operator+=(const hvec<dim, R> &v) {
    for (unsigned int i = 0; i < dim; ++i) {
  ...
  // works with different vector data types
  // play with caution
  template <typename R> hvec<dim, T> operator*(const R &s) const {
    hvec<dim, T> tmp(*this);
    for (unsigned int i = 0; i < dim; ++i) {
  ...
  // works with different vector data types
  // play with caution
  template <typename R> hvec<dim, T> &operator*=(const R &s) {
    for (unsigned int i = 0; i < dim; ++i) {
  ...
  // play with caution
  template <typename R> double dotprod(const hvec<dim, R> &v) const {
    double tmp{0};
    for (unsigned int i = 0; i < dim; ++i) {
  ...
  // osteam function
  friend std::ostream &operator<<(std::ostream &os, const hvec<dim, T> &v) {
    for (unsigned int i = 0; i < dim; ++i) {
      os << v[i] << "\t";
    }
    os << std::endl;
