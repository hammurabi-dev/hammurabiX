...
void Grid_int::build_grid (const Param *par){
    if (par->grid_int.do_dm) {
        dm_map.SetNside (par->grid_int.nside_sim,
                         RING);
    }
    if (par->grid_int.do_sync.back()) {
        Is_map.SetNside (par->grid_int.nside_sim,
                         RING);
        Qs_map.SetNside (par->grid_int.nside_sim,
                         RING);
        Us_map.SetNside (par->grid_int.nside_sim,
                         RING);
    }
    if (par->grid_int.do_fd or par->grid_int.do_sync.back()) {
        fd_map.SetNside (par->grid_int.nside_sim,
                         RING);
    }
}
