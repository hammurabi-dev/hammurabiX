...
double FEreg::density_blur (const vec3_t<double> &pos,
    ...
    // gaussian blur scale
    double blur_scale_x {(par->grid_fereg.x_max-par->grid_fereg.x_min)/(par->grid_fereg.nx*CGS_U_kpc)};
...
double FEreg::read_grid (const vec3_t<double> &pos,
    ...
    double fe;
    if (xl+1<par->grid_fereg.nx and yl+1<par->grid_fereg.ny and zl+1<par->grid_fereg.nz) {
        std::size_t idx1 {toolkit::index3d(par->grid_fereg.nx,par->grid_fereg.ny,par->grid_fereg.nz,xl,yl,zl)};
        std::size_t idx2 {toolkit::index3d(par->grid_fereg.nx,par->grid_fereg.ny,par->grid_fereg.nz,xl,yl,zl+1)};
        double i1 {grid->fe[idx1]*(1.-zd) + grid->fe[idx2]*zd};
        idx1 = toolkit::index3d(par->grid_fereg.nx,par->grid_fereg.ny,par->grid_fereg.nz,xl,yl+1,zl);
        idx2 = toolkit::index3d(par->grid_fereg.nx,par->grid_fereg.ny,par->grid_fereg.nz,xl,yl+1,zl+1);
        double i2 {grid->fe[idx1]*(1-zd) + grid->fe[idx2]*zd};
        idx1 = toolkit::index3d(par->grid_fereg.nx,par->grid_fereg.ny,par->grid_fereg.nz,xl+1,yl,zl);
        idx2 = toolkit::index3d(par->grid_fereg.nx,par->grid_fereg.ny,par->grid_fereg.nz,xl+1,yl,zl+1);
        double j1 {grid->fe[idx1]*(1-zd) + grid->fe[idx2]*zd};
        idx1 = toolkit::index3d(par->grid_fereg.nx,par->grid_fereg.ny,par->grid_fereg.nz,xl+1,yl+1,zl);
        idx2 = toolkit::index3d(par->grid_fereg.nx,par->grid_fereg.ny,par->grid_fereg.nz,xl+1,yl+1,zl+1);
