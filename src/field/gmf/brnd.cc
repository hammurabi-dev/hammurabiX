...
hvec<3,double> Brnd::get_brnd (const hvec<3,double> &pos,
                               const Param *par,
...
hvec<3,double> Brnd::read_grid (const hvec<3,double> &pos,
                                const Param *par,
                                const Grid_brnd *grid) const{
    double tmp {(par->grid_brnd.nx-1)*(pos[0]-par->grid_brnd.x_min)/(par->grid_brnd.x_max-par->grid_brnd.x_min)};
    if (tmp<0 or tmp>par->grid_brnd.nx-1) { return hvec<3,double> {0.,0.,0.};}
    decltype(par->grid_brnd.nx) xl {(std::size_t)std::floor(tmp)};
    const double xd {tmp - xl};
    
    tmp = (par->grid_brnd.ny-1)*(pos[1]-par->grid_brnd.y_min)/(par->grid_brnd.y_max-par->grid_brnd.y_min);
    if (tmp<0 or tmp>par->grid_brnd.ny-1) { return hvec<3,double> {0.,0.,0.};}
    decltype(par->grid_brnd.nx) yl {(std::size_t)std::floor(tmp)};
    const double yd {tmp - yl};
    
    tmp = (par->grid_brnd.nz-1)*(pos[2]-par->grid_brnd.z_min)/(par->grid_brnd.z_max-par->grid_brnd.z_min);
    if (tmp<0 or tmp>par->grid_brnd.nz-1) { return hvec<3,double> {0.,0.,0.};}
