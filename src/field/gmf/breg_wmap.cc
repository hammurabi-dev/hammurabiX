...
hvec<3,double> Breg_wmap::breg (const hvec<3,double> &pos,
    ...
    const double psi {par->breg_wmap.psi0 + par->breg_wmap.psi1*std::log(r/(8.*CGS_U_kpc))};
    const double chi {par->breg_wmap.chi0*std::tanh(pos[2]/(1.*CGS_U_kpc))};
    return hvec<3,double> {
        par->breg_wmap.b0*(std::sin(psi)*std::cos(chi)*cos_p - std::cos(psi)*std::cos(chi)*sin_p),
        par->breg_wmap.b0*(std::sin(psi)*std::cos(chi)*sin_p + std::cos(psi)*std::cos(chi)*cos_p),
