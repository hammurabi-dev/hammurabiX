...
hvec<3,double> Breg::get_breg (const hvec<3,double> &pos,
                               const Param *par,
...
hvec<3,double> Breg::breg (const hvec<3,double> &,
                           const Param *) const{
    return hvec<3,double> {0.,0.,0.};
}

hvec<3,double> Breg::read_grid (const hvec<3,double> &pos,
                                const Param *par,
                                const Grid_breg *grid) const{
    double tmp {(par->grid_breg.nx-1)*(pos[0]-par->grid_breg.x_min)/(par->grid_breg.x_max-par->grid_breg.x_min)};
    if (tmp<0 or tmp>par->grid_breg.nx-1) {return hvec<3,double>(0.,0.,0.);}
    decltype(par->grid_breg.nx) xl {(std::size_t)floor(tmp)};
    const double xd {tmp - xl};
    tmp = (par->grid_breg.ny-1)*(pos[1]-par->grid_breg.y_min)/(par->grid_breg.y_max-par->grid_breg.y_min);
    if (tmp<0 or tmp>par->grid_breg.ny-1) { return hvec<3,double>(0.,0.,0.);}
    decltype(par->grid_breg.nx) yl {(std::size_t)floor(tmp)};
    const double yd {tmp - yl};
    tmp = (par->grid_breg.nz-1)*(pos[2]-par->grid_breg.z_min)/(par->grid_breg.z_max-par->grid_breg.z_min);
    if (tmp<0 or tmp>par->grid_breg.nz-1) { return hvec<3,double>(0.,0.,0.);}
