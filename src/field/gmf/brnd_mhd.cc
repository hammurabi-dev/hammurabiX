...
void Brnd_mhd::write_grid (const Param *par,
    ...
    const hvec<3,double> B {breg->get_breg(par->SunPosition,par,gbreg)};
    // physical dk^3
...
#pragma omp parallel for schedule(static) // DO NOT CHANGE SCHEDULE TYPE
    ...
    for (decltype(par->grid_brnd.nx) i=0;i<par->grid_brnd.nx;++i) {
        ...
        if (i>=(par->grid_brnd.nx+1)/2) k[0] -= CGS_U_kpc*par->grid_brnd.nx/lx;
        ...
        for (decltype(par->grid_brnd.ny) j=0;j<par->grid_brnd.ny;++j) {
            k[1] = CGS_U_kpc*j/ly;
            if (j>=(par->grid_brnd.ny+1)/2) k[1] -= CGS_U_kpc*par->grid_brnd.ny/ly;
            ...
            for (decltype(par->grid_brnd.nz) l=0;l<par->grid_brnd.nz;++l) {
                ...
                k[2] = CGS_U_kpc*l/lz;
                if (l>=(par->grid_brnd.nz+1)/2) k[2] -= CGS_U_kpc*par->grid_brnd.nz/lz;
...
hvec<3,double> Brnd_mhd::eminus (const hvec<3,double> &b,
                                 const hvec<3,double> &k) const{
    hvec<3,double> tmp {((k.versor()).crossprod(b.versor())).crossprod(k.versor())};
    if (tmp.lengthsq()<1e-12){
        return tmp;
    }
    else {
