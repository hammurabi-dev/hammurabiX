...
void Grid_fernd::build_grid(const Param *par) {
  ...
  fe_k[0][0] = 0;
  fe_k[0][1] = 0; // 0th term should be zero
                  // DFT plan
#ifdef _OPENMP
  fftw_init_threads();
  fftw_plan_with_nthreads(omp_get_max_threads());
#endif
  plan_fe_bw = fftw_plan_dft_3d(par->grid_fernd.nx, par->grid_fernd.ny,
                                par->grid_fernd.nz, fe_k, fe_k, FFTW_BACKWARD,
                                FFTW_ESTIMATE);
}
...
void Grid_fernd::export_grid(const Param *par) {
  ...
  double tmp;
  for (decltype(par->grid_fernd.full_size) i = 0;
       i != par->grid_fernd.full_size; ++i) {
    assert(!output.eof());
    tmp = fe[i];
