...
double CRE::flux(const hvec<3, double> &, const Param *, const double &) const {
  throw std::runtime_error("wrong inheritance");
}

double CRE::get_emissivity_t(const hvec<3, double> &, const Param *,
                             const Grid_cre *, const double &) const {
  throw std::runtime_error("wrong inheritance");
}

double CRE::get_emissivity_p(const hvec<3, double> &, const Param *,
                             const Grid_cre *, const double &) const {
  throw std::runtime_error("wrong inheritance");
}

// use bilinear/trilinear interpolationi according to the dimension of CRE flux
// grid
double CRE::read_grid(const std::size_t &Eidx, const hvec<3, double> &pos,
  ...
  double cre;
  if (xl + 1 < par->grid_cre.nx and yl + 1 < par->grid_cre.ny and
      zl + 1 < par->grid_cre.nz) {
    ...
    std::size_t idx2{toolkit::index4d(par->grid_cre.nE, par->grid_cre.nx,
                                      par->grid_cre.ny, par->grid_cre.nz, Eidx,
                                      xl, yl, zl + 1)};
    const double i1{grid->cre_flux[idx1] * (1. - zd) +
                    grid->cre_flux[idx2] * zd};
    idx1 =
        toolkit::index4d(par->grid_cre.nE, par->grid_cre.nx, par->grid_cre.ny,
                         par->grid_cre.nz, Eidx, xl, yl + 1, zl);
    idx2 =
        toolkit::index4d(par->grid_cre.nE, par->grid_cre.nx, par->grid_cre.ny,
                         par->grid_cre.nz, Eidx, xl, yl + 1, zl + 1);
    const double i2{grid->cre_flux[idx1] * (1 - zd) +
                    grid->cre_flux[idx2] * zd};
    idx1 =
        toolkit::index4d(par->grid_cre.nE, par->grid_cre.nx, par->grid_cre.ny,
    ...
    const double j1{grid->cre_flux[idx1] * (1 - zd) +
                    grid->cre_flux[idx2] * zd};
    idx1 =
        toolkit::index4d(par->grid_cre.nE, par->grid_cre.nx, par->grid_cre.ny,
    ...
    const double j2{grid->cre_flux[idx1] * (1 - zd) +
                    grid->cre_flux[idx2] * zd};
