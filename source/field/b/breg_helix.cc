...
//Create helical magnetic field from rmin to rmax with form (bx cos(phi), by sin(phi), bz phi)
Hamvec<3, ham_float> Breg_helix::write_field(const Hamvec<3, ham_float> &pos,
                                             const Param *par) const {

	const ham_float r{
		sqrt(pos[0]*pos[0] + pos[1]*pos[1])}; //radius in cylindrical coordinates
	const ham_float phi{atan2(pos[1],pos[0])+M_PI/2.0}; //azimuthal angle in cylindrical coordinates
