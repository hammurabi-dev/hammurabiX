...
Hamvec<3, ham_float> Breg_jf12::write_field(const Hamvec<3, ham_float> &pos,
	...
	const ham_float rmin = 5.*cgs::kpc; //outer boundary of the molecular ring region
	const ham_float rcent = 3.*cgs::kpc; //inner boundary of the molecular ring region (field is zero within this region)
	const ham_float f[8] = {0.130,0.165,0.094,0.122,0.13,0.118,0.084,0.156}; //fractions of circumference spanned by each spiral, sums to unity
	const ham_float rc_B[8] = {5.1,6.3,7.1,8.3,9.8,11.4,12.7,15.5}; // the radii where the spiral arm boundaries cross the negative x-axis
	...
	if (r > Rmax || rho < rho_GC){
        ...
        //------------------------------------------------------------------------------
	//DISK COMPONENT (8 spiral regions, 7 free parameters with 8th set to conserve flux)
	...
	const ham_float B0 = (rmin/r); //
       ...
       const ham_float zprofile{ 1./(1+exp(-2./w_disk*(std::fabs(z)-h_disk)))};
       
       //printf("%g, %g \n", z, zprofile);
       ham_float B_cyl_disk[3] = {0,0,0}; // the disk field in cylindrical coordinates
       ...
       if ( (r > rcent ))  //disk field zero elsewhere
       {
	       if ( r < rmin) { //circular field in molecular ring
		       ...
		       ham_float bv_B[8] = {b_arm_1, b_arm_2, b_arm_3, b_arm_4, b_arm_5, b_arm_6, b_arm_7, 0.};
		       ham_float b8 = 0.;

		       for (int i=0; i<7; i++){ b8 -= f[i]*bv_B[i]/f[7]; }
		       bv_B[7] = b8;
	       
		       //iteratively figure out which spiral arm the current coordinates (r.phi) correspond to
		       ham_float b_disk = 0.;
		       ...
		       for (int i=7; i>=0; i--){ if (r_negx < rc_B[i]*cgs::kpc) { b_disk = bv_B[i]; } }    // "region 8,7,6,..,2"
       ...
       B_h = b1*(1. - 1./(1.+exp(-2./wh*(r-rh))))*exp(-(std::fabs(z))/(z0)); // vertical exponential fall-off
       ...
       ham_float Xtheta = 0.;
       ham_float rp_X = 0.; //the mid-plane radius for the field line that pass through r
       ham_float B_X = 0.;
       ham_float r_sign = 1.; // +1 for north, -1 for south
       if (z<0) { r_sign = -1.;} 

       //dividing line between region with constat elevation angle, and the interior:
       ...
       if (r<rc_X){ // interior region, with varying elevation angle
		...
		Xtheta = atan( std::abs(z)/ (r-rp_X) );   // modified elevation angle in interior region
		//printf("Xtheta %g at z %g , r %g , rc_X %g \n",Xtheta, z,r,rc_X);
		if (z==0.){Xtheta=M_PI/2.;}      // to avoid some NaN
										}
