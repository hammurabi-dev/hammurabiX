...
ham_float CREmodel::read_model(const Hamvec<3, ham_float> &, const ham_float &,
                               const Param *) const {
  return 0.;
}

ham_float CREmodel::flux_norm(const Hamvec<3, ham_float> &,
                              const Param *) const {
  throw std::runtime_error("wrong inheritance");
}

ham_float CREmodel::flux_idx(const Hamvec<3, ham_float> &,
                             const Param *) const {
  throw std::runtime_error("wrong inheritance");
}

ham_float CREmodel::spatial_profile(const Hamvec<3, ham_float> &,
                                    const Param *) const {
  throw std::runtime_error("wrong inheritance");
}

ham_float CREmodel::read_grid(const ham_uint &idx, const Grid_cre *grid) const {
...
ham_float CREmodel::read_grid(const Hamvec<3, ham_float> &pos,
                              const Hamvec<1, ham_float> &eng, const Param *par,
                              const Grid_cre *grid) const {
  // linear interpolation in log(E)
  ham_float tmp{std::log(eng[0] / par->grid_cre.e_min) /
...
void CREmodel::write_grid(const Param *par, Grid_cre *grid) const {
    ...
    for (decltype(par->grid_cre.ny) j = 0; j != par->grid_cre.ny; ++j) {
      ...
      for (decltype(par->grid_cre.nz) k = 0; k != par->grid_cre.nz; ++k) {
        ...
        for (decltype(par->grid_cre.ne) m = 0; m != par->grid_cre.ne; ++m) {
          ...
          grid->cre_flux[idx] = read_model(gc_pos, e, par);
        }
      }
    }
  }
